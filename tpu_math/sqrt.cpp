...
static bool verify(u16 *ofmap_data, u16 *ref_data, u16 *ifmap, u64 ifmap_shape_size,
  ...
  for (u64 i = 0; i < size; i++) {
    ...
    if (!is_close) {
      fprintf(
          stderr,
          "comparing failed at ofmap_data[%lu](input:%e), got %x, exp %x, fp32: got %e exp %e\n", i,
          convert_bf16_fp32(ifmap[i]), ofmap_data_bf16, ref, ofmap_data_f, ref_f);
      exit(-1);
    }
  }
 ...
 */
int bf16_emit_sqrt(bmctx_t *ctx, bmk_ctx_t *bmk, tl_t *tl_ifmap, tl_t *tl_buf, tl_t *tl_ofmap_bf16,
                   tl_t *tl_ofmap_u8) {
  ...
  tl_t *tl_table_answer = alloc_tl_bf16(bmk, table_shape, fmt, CTRL_AL);
  tl_t *tl_table_answer_mantissa = alloc_tl_bf16(bmk, table_shape, fmt, CTRL_AL);

  prepare_put_bf16_tensor_g2l(ctx, bmk, tl_table_answer, table_data, fmt, &copy_p2);
  prepare_put_bf16_tensor_g2l(ctx, bmk, tl_table_answer_mantissa, table_data_mantissa, fmt,
                              &copy_p3);
  ...
  p1.relu_enable = 0;
  p1.bf16_enable = 1;
  bmk1880v2_tiu_bf16_element_wise_mul(bmk, &p1);
...
void test_tl_int8_lut_bf16(bmctx_t *ctx, bmk_ctx_t *bmk, u32 input_n, u32 input_c, u32 input_h,
                           u32 input_w) {
  // TODO: check more shape / align
  tl_shape_t ifmap_shape;

  ifmap_shape = {input_n, input_c, input_h, input_w};
  ...
  tl_t *tl_ifmap = alloc_tl_bf16(bmk, ifmap_shape, fmt, CTRL_AL);
  tl_t *tl_ofmap_bf16 = alloc_tl_bf16(bmk, ofmap_shape, fmt, CTRL_AL);
  tl_t *tl_buf = alloc_tl_bf16(bmk, tl_ifmap->shape, fmt, CTRL_AL);

  tl_t *tl_ofmap_u8 = nullptr;
  tl_t *out = tl_ofmap_bf16;

  if (mode == GEN_POW_20_DATA_MAX_ERROR_U8) {
    tl_ofmap_u8 = alloc_tl_bf16(bmk, ofmap_shape, FMT_U8, CTRL_AL);
    out = tl_ofmap_u8;
  }
  ...
  bf16_emit_sqrt(ctx, bmk, tl_ifmap, tl_buf, tl_ofmap_bf16, tl_ofmap_u8);
