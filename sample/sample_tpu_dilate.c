...
int test_dilate_random(bm_handle_t handle,
                       int img_w, int img_h,
                       int kh, int kw,
                       int use_real_img,
                       char *img_name,
                       char *dilate_name)
{
    ...
    if (use_real_img)
        write_bin(dilate_name, tpu_res, img_w_stride * img_h);
...
int main(int argc, char *args[])
{
    ...
    int ret = test_dilate_random(handle, img_w, img_h, kh, kw,
                                use_real_img, src_img_name,
                                dilate_name);
    if (ret) {
        printf("----- Sample Dilate Failed -----\n");
        bm_dev_free(handle);
        return -1;
    }
    printf("----- Sample Dilate Pass -----\n");
