...
static inline u8 convert_bf16_u8(u16 data);
static inline u8 _convert_bf16_u8(u16 data, int int8_rnd_md);
static inline s8 _convert_bf16_s8(u16 data, int int8_rnd_md);
static inline s8 convert_bf16_s8(u16 data);
static inline u16 convert_int8_bf16(u8 data, u8 sign);
static inline u32 convert_fp32_u32(float fp32);
static inline u32 convert_fp32_hex(float val);
static inline float convert_hex_fp32(u32 hval);

static inline float convert_bf16_fp32(u16 bf16);
static inline u16 convert_fp32_bf16(float fp32);
...
static u8 float_isnan(const float x) {
    //return isnan(x);
    return x != x;
}
...
static inline u8 _convert_bf16_u8(u16 data, int int8_rnd_md)
{
  ...
  /* convert float32 to u8*/
  f32_integer((void*)&fp32, &convert_val.ival, 2, 0, 0, int8_rnd_md);
  return (u8) convert_val.ival;
}

static inline u8 convert_bf16_u8(u16 data)
{
  return (u8) _convert_bf16_u8(data, 0);
}
...
static inline s8 _convert_bf16_s8(u16 data, int int8_rnd_md)
{
  ...
  return (s8) convert_val.ival;
}

static inline s8 convert_bf16_s8(u16 data)
{
  return (s8) _convert_bf16_s8(data, 0);
}

static inline u16 convert_int8_bf16(u8 data, u8 sign)
{
  s32 val = sign ? (s8) data : (u8) data;
...
static inline u16 convert_fp32_bf16(float fp32)
{
  ...
  u32 input = convert_val.ival;
  u32 lsb = (input >> 16) & 1;
  u32 rounding_bias = 0x7fff + lsb;
  input += rounding_bias;
  convert_val.bf16[1] = (u16) (input >> 16);
...
static inline u8 convert_fp32_u8(float fp32)
{
  convert_int_float convert_val;
...
static inline s8 convert_fp32_s8(float fp32)
{
  convert_int_float convert_val;
...
static inline u32 convert_fp32_u32(float fp32)
{
  ...
  return (u32) convert_val.ival;
}

static inline s32 convert_fp32_s32(float fp32)
{
  convert_int_float convert_val;
  f32_integer((void*)&fp32, &convert_val.ival, 0, 0, 1, 0);
  return (s32) convert_val.ival;
}
...
static inline void flt2int_flt(float x, unsigned long * integer_part, float * sub_part, u8 sign)
{
...
inline static int flt2int(float ifval, int int8_rnd_md)
{
    ...
    u8 isPositive = ((tempIfval.intNum & 0x80000000UL) == 0x80000000UL) ? false : true ;
