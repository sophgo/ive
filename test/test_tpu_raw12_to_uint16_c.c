...
extern void fill_raw12(unsigned char* input, int len);

extern void raw12_to_uint16_cpu(unsigned char *input_data, uint16_t *output_cpu, int w, int h);
...
static int raw12_to_uint16_tpu(unsigned char* input, uint16_t* output, int height, int width, bm_handle_t handle) {
    ...
    bm_status_t ret = tpu_raw12_to_uint16(handle, input_dev_mem, output_dev_mem, width, height);
    if (ret != BM_SUCCESS) {
        printf("tpu_raw12_to_uint16 API process failed\n");
...
int main(int argc, char* args[]) {
    ...
    if (argc == 2 && atoi(args[1]) == -1) {
        ...
        printf("%s \n", args[0]);
        printf("%s 2\n", args[0]);
        printf("%s 2 1 0 1344 2412 \n", args[0]);
        printf("%s 1 1 0 1344 4824\n", args[0]);
        return 0;
    }
