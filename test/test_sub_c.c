...
int main(int argc, char **argv) {
  ...
  for (size_t i = 0; i < neon_turn; i++) {
    ...
    uint8x16_t min = vminq_u8(val, val2);
    val = vmaxq_u8(val, val2);
    uint8x16_t result = vsubq_u8(val, min);
  ...
  if (total_run == 1) {
    ...
    CVI_IVE_WriteImage(handle, "test_sub_c.png", &dst);
  }
#ifdef __ARM_ARCH
  else {
    printf("OOO %-10s %10lu %10lu %10lu\n", "SUB", elapsed_tpu, elapsed_neon, elapsed_cpu);
  }
#endif
