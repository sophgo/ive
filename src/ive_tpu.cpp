...
static int get_channel_info(int size[3], int stride[3], int h[3], int w[3], int *channel,
...
int set_blend_Image_param(Image *img, PIXEL_FORMAT_E img_format, int width,
                          int *w_stride, int height)
{
    int channel_stride[3] = {0}, stride[3] = {0}, w[3] = {0}, h[3] = {0}, channel = 3;
    int ret = 0;

    if (get_channel_info(channel_stride, stride, h, w, &channel, width, height, img_format) != 0) {
    ...
    if (w_stride != NULL) {
        for (int i = 0; i < img->channel; i++) {
            stride[i] = w_stride[i];
            channel_stride[i] = w_stride[i] * h[i];
        }
    }
...
static void process_uv_plane(unsigned char *blend_uv_base,
                             ...
                             unsigned char *right_uv_base,
                             int format,
                             int bheight, int bwidth, int uv_bstride,
                             int lheight, int uv_lstride,
                             int rheight, int rwidth, int uv_rstride,
                             unsigned char *wgt, TPU_BLEND_WGT_MODE wgt_mode,
    ...
    if (wgt_mode == WGT_YUV_SHARE) {
        ...
        for (int y = 0; y < uv_bheight; y++) {
            for (int x = 0; x < overlay_uv_w; x++) {
                ...
                wgt_uv[y * overlay_uv_w + x] = (wgt[offset00] + wgt[offset01] + wgt[offset10] + wgt[offset11]) >> 2;
            }
        }
    }
    ...
    for (int y = 0; y < uv_bheight; y++) {
        for (int x = 0; x < uv_bwidth; x++) {
            ...
            if (x < (overlay_lx / 2)) {
                ...
                if (format == PIXEL_FORMAT_NV12 || format == PIXEL_FORMAT_NV21) {
                    right_u_pos = y * uv_rstride + (format == PIXEL_FORMAT_NV12 ? right_x*2 : right_x*2+1);
                    right_v_pos = y * uv_rstride + (format == PIXEL_FORMAT_NV12 ? right_x*2+1 : right_x*2);
                }
                ...
                if (wgt_mode == WGT_YUV_SHARE) {
                    ...
                    if (format == PIXEL_FORMAT_YUV_PLANAR_420) {
                        ...
                        int alpha_ux = format ==
                                PIXEL_FORMAT_NV12 ? 2 * (x - overlay_lx / 2) : 2 * (x - overlay_lx / 2) + 1;
                        int alpha_vx = format ==
                                PIXEL_FORMAT_NV12 ? 2 * (x - overlay_lx / 2) + 1 : 2 * (x - overlay_lx / 2);
...
int cpu_2way_blend(int lwidth, int lheight, int *left_stride, unsigned char *left_img,
                   ...
                   TPU_BLEND_WGT_MODE wgt_mode, int channel, int format)
{
    ...
    if (format == PIXEL_FORMAT_YUV_PLANAR_420 ||
        ...
        process_uv_plane(blend_img + bheight * bstride,
                         ...
                         right_img + rheight * rstride,
                         format, bheight, bwidth,
                         buv_stride, lheight, luv_stride,
                         rheight, rwidth, ruv_stride,
                         wgt, wgt_mode, overlay_lx, overlay_rx);
    } else {
        for (int c = 0; c < channel; c++) {
            for (int y = 0; y < bheight; y++) {
                for (int x = 0; x < bwidth; x++) {
                    if (x < overlay_lx) {
                        ...
                        unsigned char right_pixel = right_img[c * rheight * rstride + y * rstride + (x - overlay_lx)];

                        blend_img[c * bheight * bstride + y * bstride + x] = blend_pixel(left_pixel, right_pixel, alpha);
                    }
                }
            }
        }
